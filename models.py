...
class _resDiscriminator128(nn.Module):
    def __init__(self, nIn=3, nf=64, selfAtt=False):
        ...
        self.sc = []
        # first bloc
        self.bloc0 = nn.Sequential(
            spectral_norm(nn.Conv2d(nIn, nf, 3, 1, 1, bias=True)),
            nn.ReLU(),
            spectral_norm(nn.Conv2d(nf, nf, 3, 1, 1, bias=True)),
            nn.AvgPool2d(2),
        )
        self.sc0 = nn.Sequential(
            nn.AvgPool2d(2), spectral_norm(nn.Conv2d(nIn, nf, 1, bias=True))
        )
        if selfAtt:
        ...
        for i in range(4):
            ...
            self.sc.append(
                nn.Sequential(
                    nn.AvgPool2d(2), spectral_norm(
                        nn.Conv2d(nfPrev, nf, 1, bias=True))
                )
            )
        # Last Bloc
        self.blocs.append(
        ...
        for k in range(5):
            self.convA.append(
                nn.Sequential(
                    ...
                    spectral_norm(
                        nn.Conv2d(nfPrev + 1, nfNext, 3, 1, 1, bias=False)),
                )
            )
            self.convB.append(
                spectral_norm(nn.Conv2d(nfNext, nfNext, 3, 1, 1, bias=True))
            )
    ...
    def forward(self, m, z, c):
        ...
        for k in range(5):
            ...
            h = self.convA[k](
                torch.cat(
                    (
                        F.relu(self.normA[k](x) * self.gammaA[k]
                               (c) + self.betaA[k](c)),
                        F.avg_pool2d(m, kernel_size=mask_ratio),
                    ),
                    1,
                )
            )
            h = self.convB[k](
                F.relu(self.normB[k](h) * self.gammaB[k](c) + self.betaB[k](c))
            )
            x = h + self.sc[k](x)
            mask_ratio = mask_ratio // 2
        x = self.convOut(
            torch.cat(
                (F.relu(self.normOut(x) * self.gammaOut(c) + self.betaOut(c)), m), 1
            )
        )
        x = torch.tanh(x)
        return x * m
...
class _resBloc(nn.Module):
    def __init__(self, nf=128, spectralNorm=False):
        ...
        self.activationF = nn.Sequential(
            nn.InstanceNorm2d(nf, affine=True), nn.ReLU())
...
class _upConv(nn.Module):
    def __init__(self, nOut=3, nf=128, spectralNorm=False):
        ...
        self.mods = nn.Sequential(
            ...
            spectral_norm(nn.Conv2d(nf, nf // 2, 3, 1, 1, bias=False))
            if spectralNorm
            ...
            nn.ReLU(),
            nn.ReflectionPad2d(3),
            spectral_norm(nn.Conv2d(nf // 4, nOut, 7, bias=True))
            if spectralNorm
...
class _netEncM(nn.Module):
    def __init__(self, sizex=128, nIn=3, nMasks=2, nRes=5, nf=128, temperature=1):
        ...
        self.cnn = nn.Sequential(
            *([_downConv(nIn, nf)] + [_resBloc(nf=nf) for i in range(nRes)])
        )
        self.psp = nn.ModuleList(
            [
                ...
                nn.Sequential(
                    nn.AvgPool2d(sizex // 2, sizex // 2),
                    nn.Conv2d(nf, 1, 1),
                    nn.Upsample(size=sizex, mode="bilinear"),
                ),
                nn.Sequential(
                    nn.AvgPool2d(sizex // 3, sizex // 3),
                    nn.Conv2d(nf, 1, 1),
                    nn.Upsample(size=sizex, mode="bilinear"),
                ),
                nn.Sequential(
                    nn.AvgPool2d(sizex // 6, sizex // 6),
                    nn.Conv2d(nf, 1, 1),
                    nn.Upsample(size=sizex, mode="bilinear"),
                ),
            ]
...
class _netGenX(nn.Module):
    def __init__(self, sizex=128, nOut=3, nc=8, nf=64, nMasks=2, selfAtt=False):
        ...
        self.net = nn.ModuleList(
            [
                _resMaskedGenerator128(
                    nf=nf, nOut=nOut, nc=nc, selfAtt=selfAtt)
