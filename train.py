...
parser.add_argument(
    "--nx", type=int, default=3, help="number of canals of the input image"
)
parser.add_argument("--sizex", type=int, default=128,
                    help="size of the input image")
parser.add_argument("--nz", type=int, default=32,
                    help="size of the latent z vector")
parser.add_argument("--nMasks", type=int, default=2, help="number of masks")
parser.add_argument(
    "--nResM", type=int, default=3, help="number of residual blocs in netM"
)
...
parser.add_argument(
    "--useSelfAttD", action="store_true", help="use self attention for D"
)
parser.add_argument(
    "--useSelfAttG", action="store_true", help="use self attention for G"
)
parser.add_argument(
...
parser.add_argument("--nTest", type=int, default=5,
                    help="input batch size for visu")
parser.add_argument("--nIteration", type=int, default=5e4,
                    help="number of iterations")
parser.add_argument("--initOrthoGain", type=float, default=0.8)
parser.add_argument(
    "--lrG", type=float, default=1e-4, help="learning rate for G, default=1e-4"
)
parser.add_argument(
    "--lrM", type=float, default=1e-5, help="learning rate for M, default=1e-5"
)
parser.add_argument(
    "--lrD", type=float, default=1e-4, help="learning rate for D, default=1e-4"
)
parser.add_argument(
    "--lrZ", type=float, default=1e-4, help="learning rate for Z, default=1e-4"
)
parser.add_argument(
    "--gStepFreq", type=int, default=1, help="wait x steps for G updates"
)
parser.add_argument(
    "--dStepFreq", type=int, default=1, help="wait x steps for D updates"
)
parser.add_argument("--temperature", type=float,
                    default=1, help="softmax temperature")
parser.add_argument(
    "--wdecay", type=float, default=1e-4, help="weight decay for M, default=1e-4"
)
parser.add_argument(
    "--wrecZ", type=float, default=5, help="weight for z reconstruction"
)
parser.add_argument(
    "--outf", default=".", help="folder to output images and model checkpoints"
)
parser.add_argument(
    "--checkpointFreq", type=int, default=500, help="frequency of checkpoints"
)
parser.add_argument(
    "--iteration", type=int, default=0, help="iteration to load (to resume training)"
)
parser.add_argument("--manualSeed", type=int, help="manual seed")
parser.add_argument("--resume", action="store_true",
                    help="resume from last save")
parser.add_argument("--clean", action="store_true",
                    help="clean previous states")
...
if opt.dataset == "flowers":
    ...
    testset = datasets.FlowersDataset(
        opt.dataroot,
        "test",
        transforms.Compose(
            [
...
trainloader = torch.utils.data.DataLoader(
    trainset, batch_size=opt.batchSize, shuffle=True, num_workers=4, drop_last=True
)
...
netGenX = models._netGenX(
    sizex=opt.sizex,
    nOut=opt.nx,
    nc=opt.nz,
    nf=opt.nfX,
    nMasks=opt.nMasks,
    selfAtt=opt.useSelfAttG,
).to(device)
netDX = models._resDiscriminator128(nIn=opt.nx, nf=opt.nfD, selfAtt=opt.useSelfAttD).to(
    device
)
...
# Optimizer                                                                 #
#############################################################################
optimizerEncM = torch.optim.Adam(
    netEncM.parameters(),
    lr=opt.lrM,
    betas=(0, 0.9),
...
optimizerGenX = torch.optim.Adam(
    netGenX.parameters(), lr=opt.lrG, betas=(0, 0.9), amsgrad=False
)
optimizerDX = torch.optim.Adam(
    netDX.parameters(), lr=opt.lrD, betas=(0, 0.9), amsgrad=False
)

if opt.wrecZ > 0:
    netRecZ = models._netRecZ(
        sizex=opt.sizex, nIn=opt.nx, nc=opt.nz, nf=opt.nfZ, nMasks=opt.nMasks
    ).to(device)
    netRecZ.apply(weights_init_ortho)
    optimizerRecZ = torch.optim.Adam(
        netRecZ.parameters(), lr=opt.lrZ, betas=(0, 0.9), amsgrad=False
    )
...
if opt.iteration > 0:
    state = torch.load(os.path.join(
        opt.outf, "state_%05d.pth" % opt.iteration))
...
def evaluate(netEncM, loader, device, nMasks=2):
    ...
    for xLoad, mLoad in loader:
        ...
        sumScoreIoU += (
            torch.max(
                (
                    (((mPred[:, :1] >= 0.5).float() + mData) == 2)
                    .float()
                    .sum(-1)
                    .sum(-1)
                    / (((mPred[:, :1] >= 0.5).float() + mData) >= 1)
                    .float()
                    .sum(-1)
                    .sum(-1)
                ),
...
x_test, m_test = next(
    iter(
        torch.utils.data.DataLoader(
            testset, batch_size=opt.nTest, shuffle=True, num_workers=4, drop_last=True
        )
    )
)
...
out_X = torch.full(
    (opt.nMasks, opt.nTest + 1, opt.nTest + 5, opt.nx, opt.sizex, opt.sizex), -1
).to(device)
out_X[:, 1:, 0] = x_test
out_X[:, 1:, 1] = img_m_test

valloader = torch.utils.data.DataLoader(
    valset, batch_size=opt.batchSize, shuffle=True, num_workers=4, drop_last=True
)
...
while opt.iteration <= opt.nIteration:
    ...
    zData = torch.randn(
        (xData.size(0), opt.nMasks, opt.nz, 1, 1), device=device)
    ########################## Reset Nets ############################
    netEncM.zero_grad()
    netGenX.zero_grad()
    netDX.zero_grad()
    ...
    if gStep:
        ...
        xGen = (hGen + ((1 - mEnc.unsqueeze(2)) * xData.unsqueeze(1))).view(
            hGen.size(0) * hGen.size(1), hGen.size(2), hGen.size(3), hGen.size(4)
        )
        dGen = netDX(xGen)
        lossG = -dGen.mean()
        if opt.wrecZ > 0:
    ...
    if dStep:
        ...
        with torch.no_grad():
            ...
            xGen = (hGen + ((1 - mEnc.unsqueeze(2)) * xData.unsqueeze(1))).view(
                hGen.size(
                    0) * hGen.size(1), hGen.size(2), hGen.size(3), hGen.size(4)
            )
        dPosX = netDX(xReal)
        dNegX = netDX(xGen)
        err_dPosX = -1 + dPosX
    ...
    if opt.iteration % opt.checkpointFreq == 0:
        ...
        with torch.no_grad():
            ...
            out_X[:, 1:, 2] = ((out_X[:, 1:, 3] < 0).float() * -1) + (
                out_X[:, 1:, 3] > 0
            ).float()
            out_X[:, 1:, 4] = (
                netGenX(mEnc_test, z_test)
                + ((1 - mEnc_test.unsqueeze(2)) * x_test.unsqueeze(1))
            ).transpose(0, 1)
            for k in range(opt.nMasks):
                for i in range(opt.nTest):
                    zx_test = z_test.clone()
                    zx_test[:, k] = zn_test[i]
                    out_X[k, 1:, i + 5] = netGenX(mEnc_test, zx_test)[:, k] + (
                        (1 - mEnc_test[:, k: k + 1]) * x_test
                    )
            scoreAccTrain, scoreIoUTrain = evaluate(
                netEncM, trainloader, device, opt.nMasks
            )
            scoreAccVal, scoreIoUVal = evaluate(
                netEncM, valloader, device, opt.nMasks)
            if not opt.silent:
            ...
            try:
                vutils.save_image(
                    out_X.view(-1, opt.nx, opt.sizex, opt.sizex),
                    os.path.join(opt.outf, "out_%05d.png" % opt.iteration),
                    normalize=True,
                    range=(-1, 1),
                    nrow=opt.nTest + 5,
                )
            ...
            try:
                os.remove(
                    os.path.join(
                        opt.outf,
                        "state_%05d.pth" % (
                            opt.iteration - opt.checkpointFreq),
                    )
                )
            except:
                pass
