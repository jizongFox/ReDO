...
parser.add_argument("--batch_size", type=int,
                    default=40, help="number of images")
parser.add_argument(
    "--device", default="cpu", help="cpu | cuda:n, device to be loaded on"
)
parser.add_argument("--statePath", default=None,
                    help="path to pretrained weights")
parser.add_argument(
    "--statePathM", default=None, help="path to pretrained weights for mask predictor"
)
parser.add_argument(
    "--statePathX", default=None, help="path to pretrained weights for region generator"
)
parser.add_argument(
    "--statePathZ",
    default=None,
    help="path to pretrained weights for noise reconstruction",
)
parser.add_argument(
    "--statePathD", default=None, help="path to pretrained weights for discriminator"
)
...
if not load_options.statePath is None:
    states = torch.load(
        load_options.statePath, map_location={"cuda:0": load_options.device}
    )
    opt = states["options"]
    ...
    if "netRecZ" in states:
        ...
        netRecZ.eval()
    if "netDX" in states:
        netDX = models._resDiscriminator128(
            nIn=opt.nx, nf=opt.nfD, selfAtt=opt.useSelfAttD
        ).to(device)
...
if not load_options.statePathM is None:
    states = torch.load(
        load_options.statePathM, map_location={"cuda:0": load_options.device}
    )
    opt = states["options"]
    netEncM = models._netEncM(
        sizex=opt.sizex,
...
if not load_options.statePathX is None:
    states = torch.load(
        load_options.statePathX, map_location={"cuda:0": load_options.device}
    )
    opt = states["options"]
    netGenX = models._netGenX(
        sizex=opt.sizex,
...
if not load_options.statePathZ is None:
    states = torch.load(
        load_options.statePathZ, map_location={"cuda:0": load_options.device}
    )
    opt = states["options"]
    netRecZ = models._netRecZ(
        sizex=opt.sizex, nIn=opt.nx, nc=opt.nz, nf=opt.nfZ, nMasks=opt.nMasks
...
if not load_options.statePathD is None:
    states = torch.load(
        load_options.statePathD, map_location={"cuda:0": load_options.device}
    )
    opt = states["options"]
    netDX = models._resDiscriminator128(
        nIn=opt.nx, nf=opt.nfD, selfAtt=opt.useSelfAttD
...
if opt.dataset == "cmnist":
    dataset = datasets.CMNISTDataset(
        dataPath=load_options.dataroot, sets="train")

loader = torch.utils.data.DataLoader(
    dataset, batch_size=load_options.batch_size, shuffle=True
)
...
with torch.no_grad():
    ...
    out = torch.cat(
        (
            ...
            (mPred[:, 1:2] >= 0.5).float().expand_as(xData),
            xGen[:, 0] * 0.5 + 0.5,
            xGen[:, 1] * 0.5 + 0.5,
        ),
        1,
    ...
    torchvision.utils.save_image(
        out.view(-1, 3, 128, 128), "out.png", normalize=True, range=(0, 1), nrow=6
    )
