...
class CUBDataset(torch.utils.data.Dataset):
    def __init__(self, dataPath, sets="train", transform=transforms.ToTensor()):
        ...
        splits = np.loadtxt(os.path.join(
            dataPath, "train_val_test_split.txt"), int)
        self.files = np.loadtxt(os.path.join(
            dataPath, "images.txt"), str)[:, 1]
        if sets == "train":
    ...
    def __getitem__(self, idx):
        ...
        img = self.transform(
            Image.open(os.path.join(self.datapath, "images", filename))
        )
        if img.size(0) == 1:
            img = img.expand(3, img.size(1), img.size(2))
        seg = self.transform(
            Image.open(
                os.path.join(self.datapath, "segmentations",
                             filename[:-3] + "png")
            )
        )
        if seg.size(0) != 1:
...
class FlowersDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = self.transform(Image.open(
            os.path.join(self.datapath, "jpg", imgname)))
        seg = np.array(Image.open(os.path.join(
            self.datapath, "segmim", segname)))
        seg = 1 - (
            (seg[:, :, 0:1] == 0) +
            (seg[:, :, 1:2] == 0) + (seg[:, :, 2:3] == 254)
        )
...
class CMNISTDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        background = torch.FloatTensor(3, 1, 1).uniform_(
            0.33, 0.66).repeat(1, 128, 128)
        background[0] = background[1]
        color0 = torch.FloatTensor(3, 1, 1)
        color0.uniform_(0, 0.33)
        color0[1] = color0[2]
        color1 = torch.FloatTensor(3, 1, 1)
        color1.uniform_(0.66, 1)
        color1[0] = color1[2]
        try:
        ...
        if order == 0:
            bg[:, px0: px0 + obj0.size(1), py0: py0 + obj0.size(2)] = (
                bg[:, px0: px0 +
                    obj0.size(1), py0: py0 + obj0.size(2)] * (1 - obj0)
            ) + obj0 * color0
            bg[:, px1: px1 + obj1.size(1), py1: py1 + obj1.size(2)] = (
                bg[:, px1: px1 +
                    obj1.size(1), py1: py1 + obj1.size(2)] * (1 - obj1)
            ) + obj1 * color1
            seg[:, px0: px0 + obj0.size(1), py0: py0 + obj0.size(2)] = seg[
                :, px0: px0 + obj0.size(1), py0: py0 + obj0.size(2)
            ] * (1 - obj0)
            seg[0, px0: px0 + obj0.size(1), py0: py0 + obj0.size(2)] = (
                seg[0, px0: px0 + obj0.size(1), py0: py0 + obj0.size(2)]
                * (1 - obj0[0])
                + obj0[0]
            )
            seg[:, px1: px1 + obj1.size(1), py1: py1 + obj1.size(2)] = seg[
                :, px1: px1 + obj1.size(1), py1: py1 + obj1.size(2)
            ] * (1 - obj1)
            seg[1, px1: px1 + obj1.size(1), py1: py1 + obj1.size(2)] = (
                seg[1, px1: px1 + obj1.size(1), py1: py1 + obj1.size(2)]
                * (1 - obj1[0])
                + obj1[0]
            )
